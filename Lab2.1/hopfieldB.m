...
for i=1:size(Ud,1)
    ...
    title(sprintf('pattern %d: overlap over time (true: "%d", noise:%0.2f%)', i, j-1, p));
    ...
    fprintf('- Final overlap of pattern %d ( with "%d"): %f\n', i, j-1, o);
    figure
    subplot(1, 3, 1)
    ...
    title(sprintf('pattern %d (noise %0.2f%): overlap with "%d": %f', i, p, j-1, o));

    % plot energy function over time
    figure
    plot(1:t, es);
    title(sprintf('pattern %d: energy function over time.', i));
    ylabel('energy');
    xlabel('epoch');
end
